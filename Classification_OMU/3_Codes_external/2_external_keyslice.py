...
class OMUDataset2D(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(glob(os.path.join(args.data_dir, "external_jpg_cbct_90", "Ex_" + row.Label+ "_" + f'{row.ID:08}', row.file_name + ".jpg"))[0])
...
for args.fold in [0,1,2,3,4]:
    ...
    jpg_list = glob('/'.join(os.path.dirname(os.path.abspath(__file__)).split('/')[0:-1])+"/input_external/external_jpg_cbct_90/*/*")

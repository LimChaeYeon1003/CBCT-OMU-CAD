...
def log_csv(epoch, train_loss, val_loss, val_acc, val_auc):
    ...
    with open(os.path.join('/'.join(os.path.dirname(os.path.abspath(__file__)).split('/')[0:-1])+f'/1_Codes_pre/key_logs', args.kernel_type, f'log_{args.kernel_type}_fold{args.fold}.csv'), 'a', newline='') as f:        
        writer = csv.writer(f)

...
do
    python "$(dirname $0)/2_train_keyslice_de.py" --num_workers ${WORKERS} --n_epochs ${N_EPOCHS} --init_lr ${INIT_LR} --fold ${FOLD} --batch_size ${BATCH} --image_size ${IMG_SIZE} --kernel_type ${SAVE_NAME} --DEBUG ${DEBUG} --model ${MODEL}
done
...
do 
    ...
    echo $WEIGHT
    python "$(dirname $0)/3_inference_keyslice_de.py" --image_size ${IMG_SIZE} --kernel_type ${SAVE_NAME} --fold ${FOLD} --batch_size ${BATCH} --weight_dir ${WEIGHT} --num_workers ${WORKERS} --model ${MODEL} --DEBUG ${DEBUG}
done

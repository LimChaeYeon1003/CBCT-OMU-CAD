...
class OMUDataset3D(torch.utils.data.Dataset, Randomizable):
    ...
    def __getitem__(self, index):
        ...
        jpg_lst = [os.path.join(args.data_dir, 'train_jpg', "success_cbct_90",  f"{LABEL}_{ID:08d}", f"{f}.jpg") for f in jpg_lst] 
...
def run():
    ...
    # transform2D = albumentations.Compose([albumentations.CenterCrop(x_min=args.x_min,x_max=args.x_max,y_min=args.y_min,y_max=args.y_max,p=1)]) #(오브젝트 디텍션시 주석해제)
    transform2D = albumentations.Compose([albumentations.CenterCrop(height = args.crop_size, width = args.crop_size, p=1)]) #(오브젝트 디텍션시 주석처리)
    train_transforms = Compose([ScaleIntensity(), 
                                    ...
                                    RandAffine(prob=args.augment,
                                                ...
                                                padding_mode='border'),
    #                                     RandFlip(flip_probability=0.5, dims=[1, 1, 1]),
                                    ToTensor()])
    val_transforms = Compose([ScaleIntensity(), 
    #                           CenterSpatialCrop((350,350,60)), 
                              Resize((args.resize, args.resize * 2, args.z_resize)),
                              ToTensor()])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--df_dir", type=str, default=('/'.join(os.path.dirname(os.path.abspath(__file__)).split('/')[0:-1]))+"/1_Codes_pre/key_logs/cbct_90_noise/")

...
#DF_DIR=/1_Codes_pre/key_logs/cbct_90_noise/ # path for dataframe of keyslice selector
...
## Training for each fold
 ...
 do
     python "$(dirname $0)/train.py"  --num_workers ${WORKERS} --n_epochs ${N_EPOCHS} --init_lr ${INIT_LR} --fold ${FOLD} --batch_size ${BATCH} --crop_size ${CROP_SIZE} --resize ${RESIZE} --z_resize ${Z_RESIZE} --save_name ${SAVE_NAME} --DEBUG ${DEBUG} --SEED ${SEED} --model ${MODEL} --key_thresh ${KEY} --augment ${AUGMENT} 
 echo "${Fold} is done"
 done

# Inference for 5-folds
echo "$5-folds evaluation"
python "$(dirname $0)/eval.py"  --crop_size ${CROP_SIZE} --resize ${RESIZE} --z_resize ${Z_RESIZE} --DEBUG ${DEBUG} --batch_size ${BATCH} --num_workers ${WORKERS} --model ${MODEL} --key_thresh ${KEY} --save_name ${SAVE_NAME}

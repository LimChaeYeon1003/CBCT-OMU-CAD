...
class OMUDataset3D(torch.utils.data.Dataset, Randomizable):
    ...
    def __getitem__(self, index):
        ...
        jpg_lst = [os.path.join(args.data_dir, 'train_jpg', 'success',  f"{LABEL}_{ID:08d}", f"{f}.jpg") for f in jpg_lst] 
        ...
        if self.mode == 'test':
            ...
            return img_right, img_left, target_right, target_left, patient
        
def main(fold):
    global df, val_transforms, transform2D,dataset_valid
    ...
    if os.path.exists(save_dir) == False:
        ...
        model = medcam.inject(model, output_dir=save_dir, backend='gcam', layer='layer4', label='best', save_maps=True, return_score=True)
        model.eval()
        list_pred = []
        for (img_right, img_left, _, _, _) in tqdm(valid_loader):
    ...
    for i in tqdm(range(len(dataset_valid))):
        ...
        #             for view in ["coronal", "axial", "sagittal"]:
            ...
            raw_dict = {}
            label_dict = {}
            for side in range(0,2): # side=0 -> right, side=1 -> left
                nii_gz=glob(os.path.join(save_dir, 'layer4', f'attention_map_{2*i + side}_*'))[0]
                ...
                gcam_img = apply_transform(val_cam_transforms, gcam_img) #Resize to original img
                if side == 0: gcam_dict["right"] = gcam_img
                else: gcam_dict["left"] = gcam_img
            right_img, left_img, right_label, left_label, patient = dataset_valid[i]
            ...
            for side in ["right","left"]:
                img_list=[]
                if side == "right": pred_dict[side] = list_pred[2*i]
                else: pred_dict[side] = list_pred[2*i + 1]
                ...
                imageio.mimsave(os.path.join(save_dir,f'layer4_{view}', 
                                             f"{patient}_{side}_{pred_dict[side]}_{label_dict[side]}_{eval_dict[side]}.gif"), 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--df_dir", type=str, default='/'.join(os.path.dirname(os.path.abspath(__file__)).split('/')[0:-1])+"/1_Codes_pre/key_logs/11_22_13_DEBUG/")
    parser.add_argument("--key_thresh", type=float, default=0.5)
    parser.add_argument("--save_name", type=str, default="1122_resnet18_batch16_aug0.5_key0")
